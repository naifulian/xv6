...
DATA_DIR = Path('webui/data')
REPORT_DIR = Path('webui/reports')
    ...
    for branch_name in ('baseline', 'testing'):
        ...
        if complete_runs < 3:
            recommendations.append(
                f'[{branch_name}] add at least {3 - complete_runs} more complete round(s) with `./webui/analyze_all.sh collect --rounds 3 --resume` before using the results as formal thesis evidence.'
            )
        partial_runs = [run['run_id'] for run in dataset.get('run_statuses', []) if run.get('status') == 'partial']
        if partial_runs:
            recommendations.append(
...
def save_markdown_report(payload):
    ...
    lines.extend([
        ...
        '- `webui/data/results_long.csv`',
        '- `webui/data/round_results.csv`',
        '- `webui/data/comparison_summary.csv`',
        '- `webui/data/scheduler_summary.csv`',
        '- `webui/reports/experiment_report.md`',
        '- `webui/reports/thesis_memory_table.tex`',
        '- `webui/figures/*.png` / `webui/figures/*.pdf`',
        '',
    ])
...
def save_latex_table(payload):
    ...
    lines = [
        '% Auto-generated by webui/plot_results.py',
        '\\begin{tabular}{lccc}',
        '\\hline',
