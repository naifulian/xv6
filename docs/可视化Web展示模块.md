...
在前端刷新策略上，不应再采用“整页统一 3 秒轮询”。更合理的做法是分层刷新：
• 总览层：约 1 秒，负责策略、ticks、总进程数、free pages 等趋势型信息
• 进程主表与选中详情层：约 500 ms，负责任务管理器观感最强的运行态区域
• 事件层：约 200 ms，单独维护最近活动与最近退出摘要
• 当前版本事件仍主要来自快照差分，因此短生命周期进程的完全可见性仍有后续增强空间，但页面形态已不再停留在慢速总览看板
...
15. 验收标准
• 启动模块四后，页面不再采用统一 3 秒整页轮询；总览、进程主表/详情、事件流应分别以分层频率更新。
