...
main(int argc, char *argv[])
{
  ...
  printf("SYSMON_BEGIN interval=%d samples=%d\n", interval, samples);

  for(int seq = 0; seq < samples; seq++) {
    if(getsnapshot(&snap) < 0) {
      printf("SYSMON_ERROR stage=getsnapshot seq=%d\n", seq);
      exit(1);
    }
    if(getmemstat(&ms) < 0) {
      printf("SYSMON_ERROR stage=getmemstat seq=%d\n", seq);
      exit(1);
    }

    int count = getptable(ps, MAX_PSTAT);
    if(count < 0) {
      printf("SYSMON_ERROR stage=getptable seq=%d\n", seq);
      exit(1);
    }
